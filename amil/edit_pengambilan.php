...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $mysqli->prepare("UPDATE riwayat_pengambilan SET jumlah_terkumpul = ?, catatan = ? WHERE id = ? AND id_amil = ?");
    $stmt->bind_param("dsii", $jumlah_terkumpul, $catatan, $id_riwayat, $id_amil_login);
    ...
    if ($stmt->execute()) {
        ...
        $error_message = "Gagal memperbarui riwayat.";
    }
    $stmt->close();
}

// Ambil data riwayat saat ini
$stmt_select = $mysqli->prepare("SELECT r.jumlah_terkumpul, r.catatan, k.nama_lokasi FROM riwayat_pengambilan r JOIN kotak_infak k ON r.id_kotak_infak = k.id WHERE r.id = ? AND r.id_amil = ?");
$stmt_select->bind_param("ii", $id_riwayat, $id_amil_login);
$stmt_select->execute();
                    ...
                    <form action="edit_pengambilan.php?id=<?php echo $id_riwayat; ?>" method="POST">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control"
                                value="<?php echo htmlspecialchars($riwayat['nama_lokasi']); ?>" readonly>
                        </div>
                        <div class="mb-3">
                            <label for="jumlah_terkumpul" class="form-label">Jumlah Terkumpul (Rp)</label>
                            <input type="text" class="form-control" id="jumlah_terkumpul" name="jumlah_terkumpul"
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="catatan" name="catatan"
                                rows="3"><?php echo htmlspecialchars($riwayat['catatan']); ?></textarea>
                        </div>
                        <hr>
